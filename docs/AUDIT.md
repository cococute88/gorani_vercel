...
- CALENDAR-DIVIDEND-LIVE-UPDATE-POLYGON-DECLARED-FIX-1: 배당일정 최신화 시 Polygon declared 우선, 기존 confirmed cache 보호, estimated chip 스타일 회귀 수정. See `docs/CALENDAR_DIVIDEND_POLYGON_DECLARED_FIX1.md`.
...
- 2026-08-17 CALENDAR-EVENT-RETENTION-FIX-1: Fixed automatic/provider refresh and cloud/local hydration so omitted, empty, partial, failed, or sample-fallback results cannot delete previously persisted dividend events. Added canonical-identity retention merge, transaction-time Firestore union with revision ordering, APAM 2026-08-14/17 regression coverage, and removed temporary full-event diagnostic logging. See `docs/CALENDAR_EVENT_RETENTION_FIX1.md`.
# Calendar already-lost event recovery follow-up (PR #223)

- Preview actual-data probe confirmed Polygon still returns the declared APAM 2026-08-17 dividend and reconstructs the 2026-08-14 buy deadline without a ticker/date production seed.
- Default local cache hydration now unions the pre-portfolio legacy cache, and normalized `legacy-rtdb-import` provider events are promoted into the canonical cache idempotently.
- A fresh Firestore ticker document no longer suppresses backfill when a trusted local, legacy, or provider candidate contains canonical event identities missing from that document.
- Alert text/history is not treated as an event source; only complete provider-backed or normalized legacy event bodies are eligible.
