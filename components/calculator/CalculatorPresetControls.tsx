...
export default function CalculatorPresetControls({ type, values }: Props) {
  const { user } = useFirebaseAuth();
  const [presets, setPresets] = useState<CalculatorPreset[]>([]);
  const [selectedId, setSelectedId] = useState("");
  const [notice, setNotice] = useState("프리셋은 입력값 구조만 저장합니다.");
  ...
  const handleSave = async () => {
    ...
    const preset: CalculatorPreset = {
      ...
      type,
      name: `${type} preset ${filteredPresets.length + 1}`,
      values,
      createdAt: now,
      updatedAt: now,
    };
    ...
    setNotice("프리셋을 저장했어요.");
  };

  const handleLoad = () => {
    const preset = presets.find((item) => item.id === selectedId);
    setNotice(preset ? `${preset.name} 프리셋 구조를 불러왔어요.` : "불러올 프리셋을 선택해 주세요.");
  };
