...
export default function SnapshotHistory({ snapshots, onDelete }: Props) {
      ...
      <div className="scroll-dark overflow-x-auto">
        <table className="w-full min-w-[640px] text-[13px]">
          ...
          <tbody>
            ...
            {sorted.map((s) => (
              <tr key={s.id} className="border-b border-[#1c2426] hover:bg-white/[0.02]">
                ...
                <td className="px-3 py-2.5">
                  <div className="flex justify-end">
                    <button onClick={() => onDelete(s.id)} className="flex h-7 w-7 items-center justify-center rounded-md text-slate-400 hover:bg-white/10 hover:text-red-400">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </td>
