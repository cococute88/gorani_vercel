...
//
// PR-3 재구성: 기존 "시나리오 → 계좌 3개" 구조를 "계좌 진단 행 + 계좌 → Good/Bad 상세"로 뒤집는다.
// - 계좌 진단 행: 어느 계좌가 문제인지 한눈에 본다(SafetyAccountDiagnosis).
// - 계좌 기준 상세 아코디언: 한 계좌의 Good/Bad을 나란히 비교한다(SafetyAccountDetailAccordion).
...
// 상세 아코디언 열 라벨. 이 파일에 문자열을 두어 계좌별 상세가 Good/Bad을 비교함을 명시한다.
const BASIC_SCENARIO_LABEL = "Good 시나리오";
const STRESS_SCENARIO_LABEL = "Bad 시나리오";
    ...
    >
      <div className="min-w-0">
        ...
        <p className="mt-1 break-keep text-[13px] leading-6 text-slate-600 dark:text-slate-400">
          계좌별 진단과 상세를 계좌 기준으로 정리했습니다. 어느 계좌가 문제인지 먼저 보고, 필요할 때 상세를 펼쳐 Good·Bad을 비교합니다.
      ...
      {/* 계좌 진단 3행: 절세 / 위탁 / 통합의 Good→Bad 변화를 한눈에 본다. */}
      ...
      <p className="mt-3 break-keep text-[11.5px] leading-relaxed text-slate-500 dark:text-slate-500">
        Bad은 은퇴 직후 급락·정체에 따른 순서위험(Sequence-of-Returns Risk) 테스트입니다. 첫 인출연도 위험자산 가격 수익률 -30%, 이후 2년 0%, 초기 3년 배당률 20% 삭감을 적용합니다. 4년 차부터 가격성장률은 Normal과 같은 사용자 입력값의 85%를 적용하며, 장기성장률 65%가 영구 적용되는 시나리오가 아닙니다.
      </p>
