...
assert.ok(auth.includes('onAuthStateChanged(auth'), 'auth state listener must remain the source of truth');
assert.ok(!auth.includes('isMobileBrowser()') && auth.includes('signInWithPopup(firebaseAuth, googleProvider())'), 'mobile browsers must try popup sign-in first instead of forced redirect');
assert.ok(auth.includes('authLog("login:start"') && auth.includes('authLog("redirect-result:complete"') && auth.includes('authLog("client-state:onAuthStateChanged"'), 'auth flow must emit diagnostic logs for login, callback, and client state');
