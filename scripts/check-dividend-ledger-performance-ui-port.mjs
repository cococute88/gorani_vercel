...
// QQQ series 존재 + 분홍색 점선
assert.ok(comp.includes('dataKey="qqq"'), "절세 그래프에 QQQ series가 있어야 한다");
assert.ok(comp.includes('COLOR_QQQ = "#EC4899"'), "QQQ는 분홍색(#EC4899)이어야 한다");
const qqqLine = comp.match(/dataKey="qqq"[\s\S]*?\/>/);
assert.ok(qqqLine && qqqLine[0].includes("COLOR_QQQ"), "QQQ line은 분홍색을 사용해야 한다");
assert.ok(qqqLine && qqqLine[0].includes("strokeDasharray"), "QQQ line은 점선이어야 한다");
...
assert.ok(comp.includes("스냅샷 기반"), "스냅샷 source badge가 있어야 한다");
