...
//    SCHD/MSFT/달러/현금/예적금/기타) 단위로 합산하는지
...
// 3. 달러 / 현금 / 예적금 / 기타 분류.
check("3) 달러/현금/예적금/기타 분류가 된다", () => {
  assert.equal(classifyPerformanceGroup({ productName: "미국달러 예수금", tag: "달러" }), "달러");
  assert.equal(classifyPerformanceGroup({ productName: "RP 현금성", tag: "현금" }), "현금");
  assert.equal(classifyPerformanceGroup({ productName: "예수금" }), "현금");
  assert.equal(classifyPerformanceGroup({ productName: "정기예적금", tag: "예적금" }), "예적금");
  assert.equal(classifyPerformanceGroup({ productName: "자유적금" }), "예적금");
  ...
  // 색상 규칙이 모든 그룹에 정의되어 있어야 한다.
  for (const key of ["TQQQ", "QLD", "QQQ", "SPY", "SCHD", "MSFT", "달러", "현금", "예적금", "기타"]) {
...
check("5) 그룹 수익률이 평가금액/원금 기준으로 계산되고 원금 없으면 null 이다", () => {
  const { groups } = buildPerformanceAssetGroups([
    ...
    { productName: "현금", tag: "현금", valueKRW: 500 }, // 원금 없음 → returnPct null
  ]);
  ...
  const cash = groups.find((g) => g.key === "현금");
