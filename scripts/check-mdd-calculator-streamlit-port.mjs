...
const files = {
  ...
  input: read("components/calculator/CalculatorInputField.tsx"),
  status: read("components/calculator/CalculatorDataStatus.tsx"),
...
// 7-1. 시장 선택은 티커 입력의 보조 컨트롤로 통합한다.
const inputSection = files.mdd.slice(files.mdd.indexOf("{/* 입력 영역 */}"), files.mdd.indexOf("<CalculatorWarningPanel"));
assert(inputSection.includes("labelTrailing={marketSelector}"), "시장 선택이 티커 입력 라벨 행에 통합됨");
assert(!inputSection.includes("시장 선택</span>"), "시장 선택 전용 라벨 제거됨");
assert(!inputSection.includes("6자리 코드 또는 .KS/.KQ를 입력하세요."), "시장 선택 카드의 중복 안내 제거됨");
assert(files.mdd.includes("min-h-7") && files.mdd.includes("text-[11px]"), "시장 선택 버튼이 절제된 보조 크기 유지");

// 7-2. MDD 티커 입력은 접근성을 유지한 한 줄 compact 필드다.
assert(files.mdd.includes("compact") && files.input.includes("aria-label={label}"), "티커 입력의 시각 라벨 제거와 접근성 이름 유지");
assert(files.input.includes("h-11") && files.input.includes("items-center") && files.input.includes("min-w-0 flex-1"), "티커 입력과 시장 선택이 44px 한 줄 구조로 정렬됨");

// 7-3. 빈 가격 배열의 로딩 중간 상태는 MDD 계산/경고/오류 UI와 분리한다.
assert(files.mdd.includes("liveQuoteSucceeded && normalizedWindowUsd.prices.length >= 2"), "유효 종가 2개 이상일 때만 MDD 계산 실행");
assert(files.mdd.includes('viewState === "loading"') && files.mdd.includes('error={viewState === "loading" ? null'), "로딩 중 Warning 및 오류 패널 숨김");
assert(files.mdd.includes("라이브 시세를 불러오는 중입니다…") && files.mdd.includes("시세가 준비되면 MDD 분석을 시작합니다."), "로딩 전용 상태 안내 표시");
assert(!files.lib.includes("At least two valid close prices"), "데이터 부족 경고 한글화");
assert(files.status.includes('if (!source && !loading && !updatedAt && !extra) return null;') && files.status.includes('if (loading) return "LOADING"'), "idle/LOADING/SAMPLE/LIVE 상태가 분리됨");
