...
assert.ok(page.includes('quoteHistoryPath({ ticker, range: "3y" })'), "shared performance loader must request enough stock history");
assert.ok(page.includes("histories={{"), "page must share one performance history bundle with account charts");
assert.ok(!account.includes("quoteHistoryPath"), "account charts must not issue duplicate history requests");
...
assert.ok(allFailed.unavailableReason.includes("보유종목 과거 가격 이력"));
