...
assert.ok(comp.includes('exDateMs: new Date(`${row.exDate}T00:00:00Z`).getTime()'), 'chart rows must include a stable UTC timestamp sort/axis key');
...
assert.ok(comp.includes('tickFormatter={formatChartDate}') && comp.includes('ticks={chartTicks}'), 'date ticks must be labels over the timestamp axis');
...
].sort((a, b) => a.exDate.localeCompare(b.exDate)).map((row) => ({ ...row, exDateMs: new Date(`${row.exDate}T00:00:00Z`).getTime() }));
