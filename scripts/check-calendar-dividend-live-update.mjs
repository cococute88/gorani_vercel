...
const retention = read("lib/calendar-event-retention.ts");
...
assert.match(page, /mergeFetchedEventsWithExistingCache\(existingEvents, payload\.events\)/, "live refresh merges fetched events with existing provider cache");
...
assert.match(page, /await Promise\.all\(persistedCacheCleanupWrites\);\s*if \(cancelled \|\| requestSequence !== providerRequestSequenceRef\.current\) return;\s*firestoreCacheTickersRef\.current = firestoreCacheTickers/, "cancelled or superseded portfolio loads cannot overwrite the active cache ticker ref");
