...
import { describeScenarioRisk, describeSafety } from "../lib/asset-simulator-portfolio-ui.ts";
import { buildExitYearPlans, calculateAssetSimulatorPreview, normalizeInputs } from "../lib/asset-simulator.ts";
import { buildDefaultYearPlans, DEFAULT_SIMULATOR_INPUTS, DEFAULT_YEAR_PLANS } from "../lib/mock-asset-simulator-data.ts";
...
function countCombinedProxyShortfalls(projection: SimulatorProjection, startIndex: number): number {
  return projection.totalWithdrawRows.slice(startIndex).filter((row) => {
    const requiredMonthlyReal = Math.max(0, row.realWithdraw ?? 0) / 12;
    if (!row.isWithdraw || requiredMonthlyReal <= 0) return false;
    const availableMonthlyReal = Math.max(0, row.taxSavingMonthlyReal) + Math.max(0, row.taxableMonthlyDividendReal);
    return requiredMonthlyReal - availableMonthlyReal >= requiredMonthlyReal * 0.01;
  }).length;
}
...

{
  const projection = fixture({
    taxAssets: [999, 100, 80],
    brokerageAssets: [999, 200, 100],
    combinedAssets: [1998, 300, 180],
  });
  const withdrawalStartIndex = projection.timeline.withdrawalStartIndex;
  assert.equal(withdrawalStartIndex, 1, "fixture 의 실제 인출 시작 행은 은퇴 행 다음 행");
  const result = calculateRetirementSafety(projection);

  assert.equal(result.taxSaving.metrics.startingRealAssets, 100, "절세계좌 보존율 분모는 실제 인출 시작 행");
  assert.equal(result.brokerage.metrics.startingRealAssets, 200, "위탁계좌 보존율 분모는 실제 인출 시작 행");
  assert.equal(result.combined.metrics.startingRealAssets, 300, "통합 보존율 분모는 실제 인출 시작 행");
  assert.equal(result.taxSaving.metrics.preservationRatio, 0.8, "절세계좌 보존율 = 최종 / 실제 인출 시작");
  assert.equal(result.brokerage.metrics.preservationRatio, 0.5, "위탁계좌 보존율 = 최종 가격잔고 / 실제 인출 시작");
  assert.equal(result.combined.metrics.preservationRatio, 0.6, "통합 보존율 = 최종 / 실제 인출 시작");
}
...
{
  const result = calculateRetirementSafety(fixture({ taxAssets: [100, 100, 75, 50] })).taxSaving;
...
{
  const result = calculateRetirementSafety(fixture({ taxAssets: [100, 100, 60, 40] })).taxSaving;
...
{
  const result = calculateRetirementSafety(fixture({ taxAssets: [100, 100, 60, 30] })).taxSaving;
...
// Preview처럼 자산 보존율은 높지만 목표 대비 작은 부족(96%)이 길게 이어지는 경우,
// 부족 사실은 남기되 hard failure/34점 상한으로 잘라서는 안 된다.
{
  const result = calculateRetirementSafety(fixture({
    combinedAssets: [500, 500, 500, 500, 500],
    requiredMonthlyReal: 100,
    taxMonthlyReal: [0, 96, 96, 96, 96],
    brokerageMonthlyReal: 0,
  }), { targetMonthlyExpenseReal: 100 }).combined;
  assert.equal(result.metrics.monthlyIncomeCoverageRatio, 0.96, "작은 부족 fixture 의 평균 충당률 96%");
  assert.equal(result.metrics.shortfallYears, 4, "1% 이상 작은 부족도 생활비 미달 기간에는 유지");
  assert.equal(result.metrics.severeShortfallYears, 0, "목표 대비 10% 미만 부족은 큰 부족이 아님");
  assert.equal(result.metrics.failed, false, "96% 충당률 장기 부족은 hard failure가 아님");
  assert.ok(result.score > 80, "높은 보존율과 96% 충당률은 34점 상한 대신 실제 종합 점수를 사용");
  assert.equal(describeSafety(result).gradeLabel, "안전", "작은 부족만 있는 높은 보존율 결과를 위험으로 고정하지 않음");
}

// 큰 부족은 이전처럼 장기 현금흐름 위험으로 유지한다.
{
  const result = calculateRetirementSafety(fixture({
    combinedAssets: [500, 500, 500, 500, 500],
    requiredMonthlyReal: 100,
    taxMonthlyReal: [0, 75, 75, 75, 75],
    brokerageMonthlyReal: 0,
  }), { targetMonthlyExpenseReal: 100 }).combined;
  assert.equal(result.metrics.severeShortfallYears, 4, "목표 대비 10% 이상 부족은 큰 부족으로 집계");
  assert.equal(result.metrics.failed, true, "큰 부족이 장기화되면 hard failure 유지");
  assert.equal(result.metrics.failureReason, "INCOME_SHORTAGE", "큰 부족 hard failure 코드");
  assert.equal(result.score, 34, "큰 부족 hard failure만 34점 상한 적용");
}

{
  const good = calculateRetirementSafety(fixture({
    combinedAssets: [500, 500, 500, 500, 500],
    requiredMonthlyReal: 100,
    taxMonthlyReal: [0, 100, 100, 100, 100],
    brokerageMonthlyReal: 0,
  }), { targetMonthlyExpenseReal: 100 }).combined;
  const normal = calculateRetirementSafety(fixture({
    combinedAssets: [500, 500, 500, 500, 500],
    requiredMonthlyReal: 100,
    taxMonthlyReal: [0, 96, 96, 96, 96],
    brokerageMonthlyReal: 0,
  }), { targetMonthlyExpenseReal: 100 }).combined;
  const bad = calculateRetirementSafety(fixture({
    combinedAssets: [500, 300, 260, 220, 195],
    requiredMonthlyReal: 100,
    taxMonthlyReal: [0, 91, 91, 91, 91],
    brokerageMonthlyReal: 0,
  }), { targetMonthlyExpenseReal: 100 }).combined;
  const risk = describeScenarioRisk(good, normal, bad);
  assert.equal(describeSafety(good).gradeLabel, "안전", "Good 개별 판단 유지");
  assert.equal(describeSafety(normal).gradeLabel, "안전", "Normal의 작은 부족은 위험으로 고정하지 않음");
  assert.notEqual(describeSafety(bad).gradeLabel, "안전", "낮은 Bad 보존율은 개별 판단에 반영");
  assert.equal(risk.label, "주의", "큰 부족·고갈이 없는 낮은 Bad 보존율은 종합 주의");
}

{
  ...
  assert.equal(result.metrics.shortfallYears, 2, "생활비 부족 metrics는 유지");
  assert.equal(result.metrics.failed, false, "목표 생활비 입력 전에는 장기 proxy 부족도 hard failure가 아님");
  assert.equal(result.metrics.failureReason, "NONE", "장기 proxy 부족은 실패 코드로 승격하지 않음");
  assert.notEqual(result.grade, "F", "장기 proxy 부족만으로 F가 되지 않음");
}

{
  ...
  assert.equal(result.metrics.shortfallYears, 3, "비연속 생활비 proxy 부족 3년 metrics 유지");
  assert.equal(result.metrics.consecutiveShortfallYears, 1, "비연속 부족 최장 1년");
  assert.equal(result.metrics.failed, false, "proxy 부족 총 3년도 hard failure가 아님");
  assert.equal(result.metrics.failureReason, "NONE", "proxy 부족 총 3년은 실패 코드로 승격하지 않음");
}
...

{
  ...
  assert.equal("incomeShortfallYears" in result.metrics, false, "중복 shortfall metrics 제거");
...

{
  ...
  for (const account of Object.values(result)) {
    ...
    assert.equal(account.status, "data_insufficient", "은퇴 데이터 부족 상태");
    assert.equal(account.grade, null, "은퇴 데이터 없으면 안전 등급을 확정하지 않음");
  }
}

// 실제 calculateAssetSimulatorPreview 경로를 통과하는 E2E 회귀 검사.
{
  const projection = calculateAssetSimulatorPreview(DEFAULT_SIMULATOR_INPUTS, DEFAULT_YEAR_PLANS, false);
  const result = calculateRetirementSafety(projection);
  assert.equal(result.taxSaving.status, "evaluated", "기본 일반 모드 절세계좌 평가됨");
  assert.equal(result.taxSaving.metrics.failed, false, "기본 일반 모드 절세계좌 hard failure 아님");
  assert.equal(result.combined.status, "evaluated", "기본 일반 모드 통합 평가됨");
  assert.equal(result.combined.metrics.failed, false, "기본 일반 모드 proxy 부족은 hard failure 아님");
  assert.notEqual(result.combined.grade, "F", "기본 일반 모드 통합은 가짜 부족 때문에 F가 아님");
  assert.equal(result.brokerage.status, "evaluated", "기본 입력 위탁계좌는 평가됨");
  assertScoreRange(result, "default-normal-e2e");
}

{
  const zeroBrokerage = normalizeInputs({ ...DEFAULT_SIMULATOR_INPUTS, initialTaxableDividend: 0 });
  const projection = calculateAssetSimulatorPreview(zeroBrokerage, buildDefaultYearPlans(zeroBrokerage.startYear, zeroBrokerage.years), true);
  const result = calculateRetirementSafety(projection).brokerage;
  assert.equal(result.status, "not_applicable", "위탁계좌 0원은 평가 대상 없음");
}

{
  const projection = calculateAssetSimulatorPreview(DEFAULT_SIMULATOR_INPUTS, DEFAULT_YEAR_PLANS, true);
  const result = calculateRetirementSafety(projection);
  assert.equal(result.combined.status, "evaluated", "기본 EXIT 통합 평가됨");
  assert.equal(result.combined.metrics.failed, false, "기본 EXIT에서 대기/proxy 부족은 hard failure 아님");
  assert.notEqual(result.combined.grade, "F", "기본 EXIT 통합은 proxy 부족 때문에 F가 아님");
}

{
  const inputs = normalizeInputs({ ...DEFAULT_SIMULATOR_INPUTS, initialTaxableDividend: 10000 });
  const projection = calculateAssetSimulatorPreview(inputs, buildDefaultYearPlans(inputs.startYear, inputs.years), false);
  const result = calculateRetirementSafety(projection).brokerage;
  assert.equal(result.status, "evaluated", "위탁계좌 1억원은 평가됨");
  assert.notEqual(result.grade, null, "위탁계좌 보유 시 등급 생성");
  assert.equal(typeof result.metrics.dividendsContinued, "boolean", "위탁계좌 배당 지속성 metrics 생성");
}

{
  const inputs = normalizeInputs({ ...DEFAULT_SIMULATOR_INPUTS, withdrawalDelayYears: 5 });
  const projection = calculateAssetSimulatorPreview(inputs, buildDefaultYearPlans(inputs.startYear, inputs.years), false);
  const retirementIndex = projection.timeline.retirementIndex;
  const withdrawalStartIndex = projection.timeline.withdrawalStartIndex;
  assert.notEqual(retirementIndex, null, "대기 구간 E2E 은퇴 인덱스 존재");
  assert.notEqual(withdrawalStartIndex, null, "대기 구간 E2E 인출 시작 인덱스 존재");
  const legacyCount = countCombinedProxyShortfalls(projection, retirementIndex! + 1);
  const afterWaitingCount = countCombinedProxyShortfalls(projection, withdrawalStartIndex!);
  const result = calculateRetirementSafety(projection).combined;
  assert.ok(legacyCount > afterWaitingCount, "인출 전 대기 구간에는 legacy proxy 가짜 부족이 존재");
  assert.equal(result.metrics.shortfallYears, afterWaitingCount, "대기 구간을 제외한 부족 연도만 metrics에 반영");
  assert.equal(result.metrics.failed, false, "대기 제외 후 proxy 부족은 hard failure가 아님");
}

console.log("asset simulator retirement safety checks passed (composite scores + preview E2E + no-account status + waiting exclusion)");
