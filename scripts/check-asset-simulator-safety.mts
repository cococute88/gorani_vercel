...
// 실제 calculateAssetSimulatorPreview 경로를 통과하는 E2E 회귀 검사.
{
  ...
  assert.equal(result.brokerage.status, "not_applicable", "위탁계좌 0원은 평가 대상 없음");
  assert.equal(result.brokerage.metrics.failed, false, "위탁계좌 0원은 실패가 아님");
  assert.equal(result.brokerage.metrics.failureReason, "DATA_INSUFFICIENT", "위탁계좌 0원 데이터 부족 코드");
  assert.equal(result.brokerage.grade, null, "위탁계좌 0원은 F 대신 등급 없음");
  assert.ok(result.brokerage.warnings.some((message) => message.includes("위탁계좌 데이터")), "위탁계좌 미보유 안내 문구");
