...
// 평가 완료 F도 화면에서는 고갈위험 중심 상태로 바꾼다.
...
assert.equal(fDisplay.gradeLabel, "위험", "평가 완료된 F 는 고갈위험 상태로 표시");
assert.equal(fDisplay.toneLabel, "고갈위험 높음", "F 는 고갈위험 문구로 표시");
...
assert.match(section, /resolveEffectivePortfolioProjectionAssumptions/, "현재 holdings 기준 자동 요약 산출");
assert.match(section, /비중 100% 입력 후 계산/, "비중 불완전 시 확정 수익률 대신 확인 안내");
assert.match(section, /setResolutions\(keepActive\)/, "삭제 종목 resolver 결과 정리");
assert.match(section, /onSave/, "결과 확인 옆 저장 동작 연결");
...
assert.match(kpis, /ScenarioSummaryCard[\s\S]*월 현금흐름[\s\S]*목표 대비[\s\S]*생활비 미달/, "시나리오별 월 현금흐름 결과");
assert.match(kpis, /인출 시작 대비 실질자산 보존율/, "보존율 정의 표기");
assert.match(kpis, /가장 낮은 점수/, "종합 점수와 시나리오별 점수의 차이 안내");
assert.doesNotMatch(kpis, /하락장 손상폭/, "하락장 손상폭 KPI 제거");

const comparison = read("components/asset-simulator/SafetyScenarioComparison.tsx");
assert.match(comparison, /시나리오 비교와 자산 추이/, "중복 없는 비교 영역 표기");
assert.match(comparison, /SafetyMonthlySupplyChart/, "월 현금흐름 차트 연결");
assert.match(comparison, /계산 기준 요약/, "현재 적용 계산 기준 요약 표시");
assert.match(comparison, /저장된 사용자 가정\(클라우드\)[\s\S]*저장된 사용자 가정\(이 브라우저\)[\s\S]*현재 화면에서 적용한 가정/, "저장·현재 적용·기본 가정 출처 구분");
assert.match(comparison, /적용 총수익률[\s\S]*주가성장률[\s\S]*배당률[\s\S]*배당성장률/, "실제 projection 에 적용된 가중 가정 표시");
assert.doesNotMatch(comparison, /월 공급/, "공급 용어 제거");
assert.doesNotMatch(comparison, /고갈위험 판단/, "상단 결과 판단과 중복 제거");

const scenarioTable = read("components/asset-simulator/SafetyScenarioCompareTable.tsx");
assert.match(scenarioTable, /실제 인출 시작 시점/, "보존율 분모를 실제 인출 시작 시점으로 안내");
assert.match(scenarioTable, /가격 잔고 기준/, "위탁계좌 보존율의 가격잔고 기준 안내");

const portfolioUi = read("lib/asset-simulator-portfolio-ui.ts");
assert.match(portfolioUi, /첫 인출연도 -30% 충격/, "Bad 진단 첫 인출연도 shock 안내");
assert.match(portfolioUi, /배당률은 첫 3년만 20% 삭감/, "Bad 배당률 haircut 기간 안내");
