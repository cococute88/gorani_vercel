...
assert.match(kpis, /총 월 현금흐름[\s\S]*절세계좌 월 현금흐름[\s\S]*위탁계좌 월 현금흐름/, "월 현금흐름 중심 결과");
...
assert.match(comparison, /Good · Normal · Bad 비교/, "3개 시나리오 비교 표기");
assert.match(comparison, /월 현금흐름/, "월 현금흐름 표기");
assert.doesNotMatch(comparison, /월 공급/, "공급 용어 제거");
assert.match(comparison, /고갈위험 판단/, "고갈위험 중심 판단");
